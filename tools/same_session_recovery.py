...
from runtime_config import project_label
...
CODEX_RESUME_TIMEOUT_SECONDS = int(os.environ.get("CODEX_RESUME_TIMEOUT_SECONDS", "120"))
CODEX_RESUME_MAX_FAILURES = int(os.environ.get("CODEX_RESUME_MAX_FAILURES", "3"))
...
class CodexResumeBridge:
    ...
    def build_command(self, output_path: Path) -> list[str]:
        ...
        command_path = Path(self._codex_command)
        if command_path.suffix.lower() == ".py" and command_path.exists():
            base_command = [sys.executable, str(command_path)]
        else:
            base_command = [self._codex_command]
        return base_command + [
            "exec",
            "resume",
            self._session_id,
    ...
    def wake_same_session(self, prompt: str) -> str:
        ...
        command = self.build_command(last_message_path)
        completed = self._runner(
            ...
            text=True,
            capture_output=True,
            timeout=CODEX_RESUME_TIMEOUT_SECONDS,
        )
        try:
...
def build_recovery_prompt(issue: dict[str, Any], recovery_state: dict[str, Any], rollout_snapshot: dict[str, Any]) -> str:
    ...
    return (
        f"Resume the same Codex session that owns this {project_label()} rollout.\n"
        ...
        "- If issue.browser_probe or rollout_snapshot.browser_probe contains direct browser evidence, treat that as the primary truth for Pro/quota readiness.\n"
        "- If the browser probe says ready/response_ready/draft_with_attachment, prefer retry_now unless another concrete blocker overrides it.\n"
        "- If the browser probe says pro_limit or pro_mode_required, prefer wait unless there is concrete new evidence that Gemini is actually ready.\n"
        "- If issue.resume_hint contains a concrete wait_seconds or resume_at_iso from the Gemini UI, use it as secondary timing evidence behind direct browser probe results.\n"
        "- Python will persist wait decisions as a wake schedule and re-check the browser before relaunching the worker.\n"
...
def request_recovery_decision(
    ...
    while True:
        ...
        try:
            ...
            wake_failure_count += 1
            if wake_failure_count >= CODEX_RESUME_MAX_FAILURES:
                decision = RecoveryDecision(
                    decision_version=1,
                    workflow_status="continue",
                    diagnosis=(
                        "The Codex resume bridge could not be reached repeatedly, so Python will back off "
                        "instead of spinning forever."
                    ),
                    recovery_action=RecoveryAction(
                        kind="wait",
                        reason="Codex resume bridge was unavailable repeatedly; retry after a cooldown.",
                        delay_seconds=max(WAKE_RETRY_DELAY_SECONDS, LOOP_GUARD_BASE_DELAY_SECONDS),
                    ),
                )
                state["last_decision"] = decision.to_dict()
                state["last_issue_repeat_analysis"] = repeat_analysis
                store.save(state)
                store.record_event("assistant_decision_bridge_fallback", decision.to_dict())
                return decision
