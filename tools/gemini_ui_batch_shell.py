...
from runtime_config import (
    ...
    pass_output_path,
    prompt_path as runtime_prompt_path,
    resolve_segment_file,
    worker_window_title,
)

WINDOWS_AUTOMATION_IMPORT_ERROR: ModuleNotFoundError | None = None
try:
    import pygetwindow as gw
    import pyperclip
    import uiautomation as auto
except ModuleNotFoundError as exc:
    WINDOWS_AUTOMATION_IMPORT_ERROR = exc
    gw = None
    pyperclip = None
    auto = None
...
CHROME_LAUNCH_COMMANDS = chrome_launch_commands(GEMINI_URL)
...
def ensure_windows_automation_dependencies() -> None:
    if WINDOWS_AUTOMATION_IMPORT_ERROR is None:
        return
    missing_name = getattr(WINDOWS_AUTOMATION_IMPORT_ERROR, "name", "windows automation dependency")
    raise RuntimeError(
        f"Missing dependency '{missing_name}'. Install requirements.txt before running Gemini UI automation."
    ) from WINDOWS_AUTOMATION_IMPORT_ERROR
...
def build_items(root_dir: Path, episode: int, pass_number: int, segments: list[int]) -> list[SubtitleBatchItem]:
    ...
    for segment in segments:
        segment_path = resolve_segment_file(root_dir, episode, segment)
        output_path = pass_output_path(root_dir, episode, pass_number, segment)
...
def main() -> int:
    ...
    prompt_text = ""
    if not args.probe_browser_state:
        prompt_path = runtime_prompt_path(root_dir) if args.prompt_path is None else (
            args.prompt_path if args.prompt_path.is_absolute() else (root_dir / args.prompt_path)
        )
        prompt_path = prompt_path.resolve()
        prompt_text = prompt_path.read_text(encoding="utf-8-sig").strip()
    ensure_windows_automation_dependencies()
